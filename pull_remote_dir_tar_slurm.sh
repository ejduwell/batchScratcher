pull_remote_dir_tar_slurm() {
  ...
  local timeLimit="$5"
  local pigzThreads="${6:-8}"

  if [[ -z "$remoteHost" || -z "$remoteDirPth" || -z "$localDirPth" || -z "$slurmAccount" || -z "$timeLimit" ]]; then
    echo "ERROR: Missing args."
    echo "Usage: pull_remote_dir_tar_slurm <user@host> <remoteDirPth> <localDirPth> <slurm_account> <time_limit_DD-HH:MM:SS> [pigz_threads]"
    return 2
  fi
  ...
  # Compute remote parent/base using remote-side dirname/basename (no heredocs)
  # Use -- to prevent option-parsing if path begins with '-'
