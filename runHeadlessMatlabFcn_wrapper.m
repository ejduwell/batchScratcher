function cmdOut = runHeadlessMatlabFcn_wrapper(runScriptPth, matlabFile, projectDirPthz, varargin)
% runHeadlessMatlabJob_wrapper
%
% Launch runHeadlessMatlabFcn.sh in a NEW terminal window (non-blocking),
...
%   - Runs: runHeadlessMatlabFcn.sh <matlabFile> <projectDir1> [projectDir2 ...]
...
%
% USAGE:
%   cmdOut = runHeadlessMatlabFcn_wrapper(runScriptPth, matlabFile, projectDirPthz)
%
% INPUTS:
%   runScriptPth   : full path to runHeadlessMatlabFcn.sh
%   matlabFile     : MATLAB function/script name to run (NO .m extension)
%                   *******************************************************
%                   NOTE: This version (as opposed to runHeadlessMatlabJob) 
%                          now allows you to input parameters into the
%                          matlabFile function provided that they are
%                          strings. For example paths should function fine
%                          so long as they are surrounded by quotes within
%                          the string. However, if you want to provide
%                          other types of variables, you must either
%                          hard-code those as parameters within the
%                          matlabFile or set them within a "parameter
%                          descriptor file" (i.e. a dedicated matlab 
%                          function where you simply define parameters and 
%                          export them) and make the full path to that file
%                          your input path string. If you opt for that
%                          route, your matlabFile must be set up to expect
%                          this, call the parameter descriptor file
%                          function and unpack the parameters set within
%                          there.                          
%                   *******************************************************
%
...
if ~isfile(runScriptPth)
    error('runHeadlessMatlabFcn.sh not found: %s', runScriptPth);
end
...
logPth = fullfile(tempdir, sprintf('runHeadlessMatlabFcn_wrapper_%s.log', datestr(now,'yyyymmdd_HHMMSS_FFF')));
