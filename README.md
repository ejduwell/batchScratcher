...
- This configuration sets up the Matlab cluster profile for your remote cluster to use/accept your ssh keys generated eariler instead the default of asking for a password entry via a pop-up GUI window the first time you submit a job from within a given Matlab session.
...
|------------------------|---------------------------------------------------|
| [`submitBatchClstrJobMain.m`](https://github.com/ejduwell/batchScratcher/blob/main/submitBatchClstrJobMain.m)     | Primary script for submitting batch jobs. Effectively serves as a 'wrapper' for 'submitBatchClstrJob_v1.m'                 |
| [`submitBatchClstrJob_v1.m`](https://github.com/ejduwell/batchScratcher/blob/main/submitBatchClstrJob_v1.m)     | Main function called by 'submitBatchClstrJobMain.m' for preparing copies of a local project directory, pushing it to the cluster's scratch, and submitting scripts to run there remotely as batch jobs                 |
| [`rxivMatlabPrjctDir4RemoteJob.m`](https://github.com/ejduwell/batchScratcher/blob/main/rxivMatlabPrjctDir4RemoteJob.m)     | Main function called by 'submitBatchClstrJob_v1.m' for compressing the local project directory. Effectively serves as a wrapper for the bash function 'rxivMatlabCode_v5.sh' which compresses subselections of data contained within the folder to tar.gz while maintaining the organizational structure of the project directory              |
| [`rxivMatlabCode_v5.sh`](https://github.com/ejduwell/batchScratcher/blob/main/rxivMatlabCode_v5.sh)     | Bash function called by 'rxivMatlabPrjctDir4RemoteJob.m' for preparing compressed copies of a local project directory. Allows subselection of files in specific sub-directories and/or particular filetypes while maintaining overall directory structure                |
| [`pushTarGzToCluster_v2.m`](https://github.com/ejduwell/batchScratcher/blob/main/pushTarGzToCluster_v2.m)     | Matlab function that issues bash commands to push the compressed local copy of the project directory to the remote cluster's scratch.              |
| [`pullRemoteDirTarSlurm_v1.m`](https://github.com/ejduwell/batchScratcher/blob/main/pullRemoteDirTarSlurm_v1.m)     | Matlab wrapper function for 'pull_remote_dir_tar_slurm.sh' that compresses remote directory on cluster when job is done and pulls data back to local machine.              |
| [`pull_remote_dir_tar_slurm.sh`](https://github.com/ejduwell/batchScratcher/blob/main/pull_remote_dir_tar_slurm.sh)     | Bash function called by 'pullRemoteDirTarSlurm_v1.m' for preparing and pulling compressed copies of a remote project directory on cluster, and cleaning up the remote scratch after the job is done.                |
| [`syncDirCpy2MainDir.m`](https://github.com/ejduwell/batchScratcher/blob/main/syncDirCpy2MainDir.m)     | Matlab wrapper function for 'syncDirCpy2MainDir.sh' that syncs the output directory copy pulled down from the cluster to the temporary local folder in batchScratcher/mirror2cluster/ with the original project directory on the local machine and then cleans up the tempory local copy.              |
| [`syncDirCpy2MainDir.sh`](https://github.com/ejduwell/batchScratcher/blob/main/syncDirCpy2MainDir.sh)     | Bash function called by the syncDirCpy2MainDir.m Matlab wrapper function, that uses rsync commands to detect and sync any new files/folders in the temporary output directory copy from the cluster job with the original local project directory. Also cleans up the local copy in the temporary subdirectory in batchScratcher/mirror2cluster/ by either archiving it in specified location or deleting it if specified.              |
| [`parClusterSetup4HeadlessJobs.m`](https://github.com/ejduwell/batchScratcher/blob/main/parClusterSetup4HeadlessJobs.m)     | Matlab script for setting up the parcluster profile to be able to submit batch jobs 'headlessly' (i.e. from within a UNIX terminal without an associated Matlab GUI window) via 'runHeadlessMatlabJob.sh' and 'runHeadlessInUnixTerminal.m' .  |
| [`runHeadlessInUnixTerminal.m`](https://github.com/ejduwell/batchScratcher/blob/main/runHeadlessInUnixTerminal.m)     | Main entry-point function that allows users to submit remote batch jobs to the cluster from within the Matlab GUI, but in a manner that automatically spawns an external UNIX terminal and runs it in separate 'headless' terminal-based session (such that you can continue to work in the MATLAB GUI while it runs). |
| [`runHeadlessMatlabJob_wrapper.m`](https://github.com/ejduwell/batchScratcher/blob/main/runHeadlessMatlabJob_wrapper.m)     | Matlab wrapper function for 'runHeadlessMatlabJob.sh' called by 'runHeadlessInUnixTerminal.m' that allows users to submit remote batch jobs to the cluster from within the Matlab GUI, but that automatically spawns an external UNIX terminal and runs it in separate 'headless' terminal-based session (such that you can continue to work in the MATLAB GUI while it runs).              |
| [`runHeadlessMatlabJob.sh`](https://github.com/ejduwell/batchScratcher/blob/main/runHeadlessMatlabJob.sh)     | Bash function called by 'runHeadlessMatlabJob_wrapper.m' that allows users to submit remote batch jobs 'headlessly' (i.e. without a Matlab GUI window open) from within a UNIX terminal. Could also be called from within users' shell scripts or other non-matlab-based workflows to seamlessly integrate Matlab work run on a remote cluster into a local pipeline.             |
...
- under '%% Set Parameters', set 'matlabFile' to your main job script and add any other directory paths required to run/submit the job to 'folders2add':
```matlab
...
- Run 'runHeadlessInUnixTerminal.m' either by hitting the green 'Run' button at the top of the Matlab GUI or by running the following on the Matlab Command Window:
```matlab
runHeadlessInUnixTerminal
```
...
- Finally, if you are going to use 'runHeadlessMatlabJob.sh' or any other shell/bash functions included in **batchScratcher** in this fashion, you will likely want to add the the **batchScratcher** project folder to your permenant path.
  ...
  - Critically, this means that all **batchScrather** functions will be available from the terminal window anywhere on your machine without needing to navigate to the installation folder.
