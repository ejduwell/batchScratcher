...
| `runHeadlessInUnixTerminal.m`     | Main entry-point function that allows users to submit remote batch jobs to the cluster from within the Matlab GUI, but that automatically spawns an external UNIX terminal and runs it in separate 'headless' terminal-based session (such that you can continue to work in the MATLAB GUI while it runs).
| `runHeadlessMatlabJob_wrapper.m`     | Matlab wrapper function for 'runHeadlessMatlabJob.sh' called by 'runHeadlessInUnixTerminal.m' that allows users to submit remote batch jobs to the cluster from within the Matlab GUI, but in a manner that automatically spawns an external UNIX terminal and runs it in separate 'headless' terminal-based session (such that you can continue to work in the MATLAB GUI while it runs).              |
