...
| `runHeadlessMatlabJob.sh`     | Bash function called by 'runHeadlessMatlabJob_wrapper.m' that allows users to submit remote batch jobs 'headlessly' (i.e. without a Matlab GUI window open) from within a UNIX terminal. Could also be called from within users' shell scripts or other non-matlab-based workflows to seamlessly integrate Matlab work run on a remote cluster into a local pipeline.             |
